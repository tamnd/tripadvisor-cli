...
//	location --parent_ref-------> location (its nearest geo) --parent_ref--> ... up
//	location --ancestor_refs--->> location (every geo ancestor, City..Country) up
//	location --neighborhood_refs->> location (the sub-geos inside it) down
//	location --nearby_ref--> nearby
//
// The geo tree is walkable both ways: ancestor_refs climbs from a place to its
// city, region, and country in one expansion, and neighborhood_refs descends a
// city into the districts inside it. A host that follows these edges from any seed
// can reach the whole geo hierarchy, and from the sitemap backbone it can reach
// every location page, so the graph reconstructs the public site.
//
...
type Location struct {
	...
	LocalName        string         `json:"local_name,omitempty" table:"-"` // the name in the place's own language
	...
	Groups           []string       `json:"groups,omitempty" table:"-"` // the grouped taxonomy (cuisines, features, meal types)
	Rating           float64        `json:"rating,omitempty"`           // 0 to 5
	NumReviews       int            `json:"num_reviews,omitempty"`
	RatingCounts     map[string]int `json:"rating_counts,omitempty" table:"-"`   // histogram, keys "1".."5"
	Subratings       []Subrating    `json:"subratings,omitempty" table:"-"`      // aspect averages (service, value, ...)
	...
	Features         []string       `json:"features,omitempty" table:"-"`  // hotels/restaurants (Free Wifi, Pool, ...)
	...
	LocalAddress     string         `json:"local_address,omitempty" table:"-"` // address in the place's own language
	...
	Ancestors        []Ancestor     `json:"ancestors,omitempty" table:"-"`     // the geo lineage, nearest first
	Neighborhoods    []Neighborhood `json:"neighborhoods,omitempty" table:"-"` // the sub-geos inside this place
	...
	SeeAllPhotos     string         `json:"see_all_photos,omitempty" table:"-"`                                         // the all-photos page URL
	WriteReview      string         `json:"write_review,omitempty" table:"-"`                                           // the write-a-review page URL
	RatingImage      string         `json:"rating_image_url,omitempty" table:"-"`                                       // attribution duty
	GeoID            string         `json:"geo_id,omitempty" table:"-"`                                                 // the g-number of its nearest geo
	...
	ParentRef        string         `json:"parent_ref,omitempty" table:"-" kit:"link,kind=tripadvisor/location"`        // its nearest geo ancestor
	AncestorRefs     []string       `json:"ancestor_refs,omitempty" table:"-" kit:"link,kind=tripadvisor/location"`     // every geo ancestor up the tree
	NeighborhoodRefs []string       `json:"neighborhood_refs,omitempty" table:"-" kit:"link,kind=tripadvisor/location"` // the sub-geos down the tree
	NearbyRef        string         `json:"nearby_ref,omitempty" table:"-" kit:"link,kind=tripadvisor/nearby"`          // = "lat,lng"
}

// Ancestor is one node in a location's geo lineage: a city, region, or country
// that contains it. Each is itself a geo location, addressable by its id, so a
// crawl climbs the whole tree from any place in one expansion of ancestor_refs.
type Ancestor struct {
	ID    string `json:"id"`
	Name  string `json:"name,omitempty"`
	Level string `json:"level,omitempty"` // City, Region, Country, ...
}

// Neighborhood is a sub-geo inside a location (a district within a city). Each is
// itself a geo location, so neighborhood_refs descends the geo tree.
type Neighborhood struct {
	ID   string `json:"id"`
	Name string `json:"name,omitempty"`
}

// Subrating is one aspect score: a location's average or a review's own rating for
// an aspect such as service, value, cleanliness, location, rooms, or sleep.
type Subrating struct {
	Name  string  `json:"name"`
	Value float64 `json:"value,omitempty"` // 1 to 5
}
...
type Review struct {
	...
	Translated   bool        `json:"machine_translated,omitempty" table:"-"` // text is a machine translation
	Subratings   []Subrating `json:"subratings,omitempty" table:"-"`         // this review's per-aspect ratings
...
type Photo struct {
	...
	Blessed   bool   `json:"blessed,omitempty" table:"-"` // editor-approved
